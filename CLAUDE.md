...
- **Non-selectable rows**: Hunk header (`rowHunkHeader`) and spacer (`rowSpacer`) rows are not selectable — the cursor skips over them. Only `rowDiffPair` and `rowComment` rows are selectable. `isSelectableRow()` is the single source of truth for this check. All cursor-setting paths (`moveCursor`, `moveCursorTo`, `clickAt`, viewport jumps, scroll restore, clamp functions, `setFile`) call `snapToSelectable()` to enforce this invariant. `setFile()` starts the cursor on the first selectable row, not row 0. Relative line numbers show blank (`"   "`) for non-selectable rows. `rowOrdinals` already only increment on `rowDiffPair`, so ordinal-based movement (`moveCursorByOrdinalDelta`, `moveCursorToOrdinal`) naturally skips non-selectable rows.
...
- Any code path that sets `cursorY` must ensure it doesn't land on a non-selectable row (hunk header or spacer). Use `snapToSelectable()` after assignment. Forgetting this in a new cursor-setting path will let the cursor rest on a hunk header line, which has no meaningful line number or content for commenting.
