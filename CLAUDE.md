...
- The inline comment `commentInput` is a `textarea.Model`, not a `textinput.Model`. Submit is `ctrl+s` (bound as `keys.SubmitComment`), not `Enter` (`keys.Submit`). Tests must use `tea.KeyCtrlS` for both inline and general comment submission.
